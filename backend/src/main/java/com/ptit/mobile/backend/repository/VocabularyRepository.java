...
public interface VocabularyRepository extends JpaRepository<Vocabulary, Long> {
    ...
    @Query(value = """
        SELECT
          ...
          SUM(CASE WHEN cr.id IS NOT NULL AND cr.last_reviewed_at IS NOT NULL AND cr.next_review_date > :today AND cr.next_review_date <= :todayPlus7 THEN 1 ELSE 0 END) AS upcoming7d
        FROM vocabulary v
        ...
        """, nativeQuery = true)
    VocabHomeStatsProjection getHomeStatsForUser(@Param("userId") Long userId,
