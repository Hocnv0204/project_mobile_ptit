...
import { Table, Button, Space, Card, Breadcrumb, message, Modal, Form, Input, Select, Tag } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined, ArrowLeftOutlined, EyeOutlined } from '@ant-design/icons';
...
const VocabListPage: React.FC = () => {
  ...
  const [selectedLesson, setSelectedLesson] = useState<LessonVocab | null>(null);
  ...
  const fetchLessons = async () => {
    setLoading(true);
    try {
      const response: any = await api.get('/lesson-vocab');
  ...
  const handleDeleteLesson = async (id: number) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await api.delete(`/lesson-vocab/${id}`);
  ...
  const handleLessonModalOk = async () => {
    try {
      ...
      if (editingLesson) {
        await api.put(`/lesson-vocab/${editingLesson.id}`, values);
        message.success('Lesson updated');
      } else {
        await api.post('/lesson-vocab', values);
  ...
  const fetchVocabularies = async (lessonId: number) => {
    setLoading(true);
    try {
      const response: any = await api.get(`/lesson-vocab/${lessonId}/vocabularies`);
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          message.warning('Delete functionality depends on backend implementation.');
        } catch (error) {
          message.error('Delete failed');
        }
      },
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (editingVocab) {
        ...
        // Create
        await api.post(`/vocab/${selectedLesson?.id}/single`, values);
  ...
  const lessonColumns = [
    ...
    { 
      ...
      render: (_: any, record: LessonVocab) => (
        <Space size="middle">
          <Button icon={<EyeOutlined />} onClick={() => handleLessonSelect(record)}>View Vocabs</Button>
          <Button icon={<EditOutlined />} onClick={() => handleEditLesson(record)}>Edit</Button>
          <Button icon={<DeleteOutlined />} danger onClick={() => handleDeleteLesson(record.id)}>Delete</Button>
        </Space>
      ) 
    },
  ];
  ...
  const vocabColumns = [
    { title: 'Term', dataIndex: 'term', key: 'term', render: (text: string) => <b>{text}</b> },
    ...
    { 
      ...
      render: (_: any, record: Vocabulary) => (
        <Space size="middle">
          <Button icon={<EditOutlined />} onClick={() => handleEditVocab(record)} />
          <Button icon={<DeleteOutlined />} danger onClick={() => handleDeleteVocab(record.id)} />
        </Space>
      ) 
    },
  ];
  ...
  return (
    <div>
      <Breadcrumb style={{ marginBottom: 16 }}>
        <Breadcrumb.Item onClick={() => setSelectedLesson(null)} style={{ cursor: 'pointer' }}>Vocabulary</Breadcrumb.Item>
        {selectedLesson && <Breadcrumb.Item>{selectedLesson.name}</Breadcrumb.Item>}
      </Breadcrumb>

      {!selectedLesson ? (
        <Card title="Lesson Vocabularies" extra={<Button type="primary" icon={<PlusOutlined />} onClick={handleAddLesson}>Add Lesson</Button>}>
          <Table columns={lessonColumns} dataSource={lessons} loading={loading} rowKey="id" />
        </Card>
      ) : (
        <Card 
          title={<span><Button icon={<ArrowLeftOutlined />} onClick={() => setSelectedLesson(null)} style={{ marginRight: 8 }} /> Vocabularies in {selectedLesson.name}</span>}
          extra={<Button type="primary" icon={<PlusOutlined />} onClick={handleAddVocab}>Add Vocabulary</Button>}
        >
          <Table columns={vocabColumns} dataSource={vocabularies} loading={loading} rowKey="id" />
        </Card>
      )}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="vi" label="Vietnamese Meaning" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
      ...
      >
        <Form form={lessonForm} layout="vertical">
          <Form.Item name="name" label="Lesson Name" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
